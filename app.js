...
function loadVoices() {
  ...
  if (!voices.length) {
    ...
    renderSpeechDiagnostics("等待系统语音", `浏览器已支持朗读，但还没拿到可用声音。${getPlatformSpeechChecks()}`, "warning");
    return;
  }
...
loadVoices = async function (options = {}) {
  ...
  if (!voices.length) {
    dom.voiceSelect.innerHTML = `<option value="">正在等待可用语音...</option>`;
    dom.voiceReadyPill.textContent = "语音加载中";
    refreshVoiceHint();
    renderSpeechDiagnostics("等待系统语音", `浏览器已支持朗读，但还没拿到可用声音。${getPlatformSpeechChecks()}`, "warning");
    return;
...
speakSentenceAt = function (sentenceIndex, fallbackTried = false, attemptNonce = state.speechAttemptNonce, overrideVoice = null) {
  ...
  if (sentenceIndex >= sentences.length) {
    ...
    state.paused = false;
    dom.speechStateHint.textContent = "本章朗读结束";
    renderSpeechDiagnostics("本章朗读完成", "已经顺着当前章节读到了末尾。", "success");
    updateSpeechProgress();
    return;
  }
...
refreshVoiceHint = function () {
  ...
  dom.voiceHint.textContent = voice
    ? `当前已选：${formatVoiceOptionLabel(voice)}。${stabilityHint}${englishVoiceHint}${routingHint}`
    : `语音来自当前设备系统。不同手机、浏览器和系统语音包，看到的可选声音会不同。${stabilityHint}${englishVoiceHint}${routingHint}`;
};
